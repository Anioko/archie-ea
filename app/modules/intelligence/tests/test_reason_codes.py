...
at all). The impact criticality and recovery-objectives block added two
more -- ``no_criticality_recorded`` and ``no_recovery_objective_recorded``.
...
side, T-S1 and the maturity read helper already folded into main on the
other) is a third instance of the same class of drift, resolved here by
re-deriving the real count (35) rather than trusting either side's own
stale number.
"""
...
# maturity read helper's two additions and the impact criticality and
# recovery-objectives block's two additions.
_EXPECTED = {
