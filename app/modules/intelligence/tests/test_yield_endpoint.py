...
def test_exactly_five_intelligence_routes_registered(app):
    rules = [
        rule for rule in app.url_map.iter_rules()
        if str(rule).startswith("/api/v1/intelligence")
    ]
    assert len(rules) == 5, (
        f"expected exactly 5 /api/v1/intelligence/* rules (recompute POST, "
        f"derived GET, impact GET, risk GET, yield GET) -- found {len(rules)}: "
