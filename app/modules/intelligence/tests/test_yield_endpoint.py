...
def test_exactly_ten_intelligence_routes_registered(app):
    rules = [
        rule for rule in app.url_map.iter_rules()
        if str(rule).startswith("/api/v1/intelligence")
    ]
    assert len(rules) == 10, (
        f"expected exactly 10 /api/v1/intelligence/* rules (recompute POST, "
        f"derived GET, impact GET, risk GET, portfolio GET, programme GET, "
        f"strategy GET, accountability GET, value-streams-at-risk GET, "
        f"yield GET) -- found {len(rules)}: "
