...
def _ownership(db_session, component_id, unit_id, org_id, ownership_type="Business Owner"):
    from app.models.enterprise_intelligence import ApplicationOwnership

    row = ApplicationOwnership(
        application_id=component_id, organization_unit_id=unit_id,
        organization_id=org_id, ownership_type=ownership_type,
    )
    db_session.add(row)
    db_session.flush()
    return row
...
def _org_unit(db_session, org_id, name="Finance"):
    ...
    unit = OrganizationUnit(name=f"{name} {uuid.uuid4().hex[:6]}", organization_id=org_id)
    db_session.add(unit)
    db_session.flush()
    return unit
...
def test_owner_attaches_when_chain_resolves_and_tenant_matches(app, db_session, make_org):
    ...
    _ownership(db_session, comp.id, unit.id, org.id, ownership_type="Business Owner")
    db_session.commit()
...
def test_owner_absent_is_indistinguishable_and_cross_tenant_does_not_leak(app, db_session, make_org):
    ...
    _ownership(db_session, other_tenant_comp.id, other_unit.id, org_b.id)
    db_session.commit()
...
def test_cross_tenant_component_pointer_does_not_leak_unit_name(app, db_session, make_org):
    ...
    _ownership(db_session, foreign_comp.id, foreign_unit.id, org_b.id)
    db_session.commit()
...
def test_sec09_tenant_check_blocks_real_cross_tenant_resolution(app, db_session, make_org):
    ...
    _ownership(db_session, comp.id, unit.id, org_b.id)
    db_session.commit()
...
def test_duplicate_component_pointer_resolves_deterministically_not_500(app, db_session, make_org):
    ...
    _ownership(db_session, comp1.id, unit.id, org.id, ownership_type="Business Owner")
    db_session.commit()
