...
``ownership_source_undecided`` reason on every call, pending a
product-manager decision recorded in the reuse register
(``docs/artifacts/reuse-register.yml``, ``ownership`` entry) plus a real
tenant-scoping fix to ``OrganizationUnit`` -- see the method's own
docstring. Five of six lenses in ``intelligence-lenses-v1.md`` are
currently answered.
"""
...
OWNERSHIP_SOURCE_UNDECIDED_REASON = validate_reason_code("ownership_source_undecided")
...
class IntelligenceQueryService:
    ...
    def accountability_for_element(element_id: int) -> Dict[str, Any]:
        ...
        WITHDRAWN pending a product-manager decision, per an external review
        of the original PR (`defect-report-PR107-v1.md`) that this
        implementation did not check against before shipping.

        The reuse register (`docs/artifacts/reuse-register.yml`, `ownership`
        entry) marks the ownership concept ``decision: pending``,
        ``decision_owner: product-manager``, ``write_path: none``, and
        explicitly forbids "a new screen that reads an owner record that
        nothing in the application writes" -- which is exactly what the
        original version of this method was. Confirmed: no production code
        path writes ``ApplicationOwnership`` or ``OrganizationUnit``
        (``git grep ApplicationOwnership(`` finds only the model, a delete
        helper, and test seeds) -- meaning the sixth lens would have been a
        permanent empty state for every real tenant even before the
        following, more urgent problem.

        The original implementation also had a real, unreviewed tenant-
        isolation gap: it fetched ``OrganizationUnit`` by
        ``organization_unit_id`` with no tenant predicate at all (that table
        carries no ``TenantMixin``/``organization_id``), so a
        cross-tenant-seeded ``organization_unit_id`` on an otherwise
        correctly-scoped ``ApplicationOwnership`` row would have leaked
        another organisation's unit name/type/head-of-unit -- not caught by
        this lens's own tests, which only exercised the element-level
        cross-tenant case. It also showed expired ownership (no ``end_date``
        filter) as current, and duplicated the tenant-checked
        ``_resolve_owners_batch``/``_sec09_tenant_check`` pattern
        ``cross_layer_impact`` already uses for a narrower version of this
        same element -> component -> ownership -> unit chain, rather than
        reusing it.

        Rather than patch a feature the register says should not exist yet,
        this withdraws the ownership read entirely -- no query against
        either table -- until the product-manager makes the record-of-truth
        decision the register asks for. The route, question card and tests
        stay in place so the lens is easy to re-enable once that decision
        lands and (separately) ``OrganizationUnit`` gets real tenant
        scoping; only the query itself is disabled.
        """
        ...
        return {
            ...
            "reasons": [OWNERSHIP_SOURCE_UNDECIDED_REASON, CAPACITY_NOT_AVAILABLE_REASON],
        }
