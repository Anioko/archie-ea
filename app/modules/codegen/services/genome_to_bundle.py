...
# Map genome "language" values to (runtime, framework) tuples.
# Keys must match the enum in architectural_genome_schema.json.
_LANGUAGE_TO_DEPLOY = {
    ...
    "sap-btp-integration": ("node20", "sap-btp"),
    "flask-nextjs": ("python3.12", "flask"),
    "flask-react": ("python3.12", "flask"),
}


def _parse_language(genome: dict) -> tuple:
    """Return (runtime, framework) from the genome's language field."""
    lang = genome.get("language", "python-fastapi")
    return _LANGUAGE_TO_DEPLOY.get(lang, ("python3.12", "fastapi"))
...
def _snake(name: str) -> str:
    ...
    Handles acronyms correctly: KPIs → kpis, APIs → apis, SQLQuery → sql_query.
    """
    name = _normalize_entity(name)
    s = re.sub(r"[^\w\s]", "", name)
    s = re.sub(r"\s+", "_", s.strip())
    # Protect trailing-s plurals on all-caps acronyms before standard conversion:
    # "KPIs" → "Kpis", "APIs" → "Apis" so they don't split as "KP_Is" / "AP_Is"
    s = re.sub(r"([A-Z]{2,})s\b", lambda m: m.group(0).capitalize(), s)
...
def genome_to_bundle(genome: dict) -> "ProductSpecBundle":
    ...
    # Build deployment def from genome language selection
    _runtime, _framework = _parse_language(genome)
    ...
    nfrs.append({"flag": "rbac", "description": "Role-based access control on all routes"})
    ...
    # Normalize multi_tenancy from genome (can be bool or object).
    # Default is DISABLED — per-solution deployments are single-tenant by design.
    # Only enable if the genome explicitly sets multi_tenancy.enabled = true.
    _mt_raw = security.get("multi_tenancy")
    if isinstance(_mt_raw, dict):
        multi_tenancy = {
            "enabled": _mt_raw.get("enabled", False),
            "strategy": _mt_raw.get("strategy", "row_level"),
            "tenant_field": _mt_raw.get("tenant_field", "organization_id"),
        }
    elif _mt_raw is True:
        multi_tenancy = {
            "enabled": True,
            "strategy": "row_level",
    ...
    # Attach behavioral context for pipeline template rendering
    bundle._behavioral = genome.get("_behavioral", {})
...
def _build_openapi(genome: dict) -> dict:
    ...
    for mod_key, mod_def in modules.items():
        ...
        # D-PASS4-5 / G6: buy/vendor modules get a minimal schema (no DB schema) and
        # vendor-stub paths instead of CRUD — skip full property generation for them.
        _is_vendor = mod_def.get("build_or_buy") in ("buy", "vendor")
        if _is_vendor:
            vendor_name = mod_def.get("vendor_name") or root
            component_schemas[schema_name] = {
                "type": "object",
                "description": f"Vendor integration schema for {vendor_name}",
                "x-vendor-integration": vendor_name,
                "properties": properties,
            }
            element_ids = mod_def.get("archimate_element_ids", [])
            x_source = element_ids[0] if element_ids else None
            trigger_snake = _snake(vendor_name)
            paths[resource_path] = {
                "get": {
                    "operationId": f"query_{root_snake}_vendor",
                    "summary": f"Query {vendor_name} vendor records",
                    "tags": [schema_name],
                    "x-vendor-integration": vendor_name,
                    "responses": {"200": {"description": "OK"}},
                },
            }
            paths[f"{resource_path}/health"] = {
                "get": {
                    "operationId": f"{root_snake}_vendor_health",
                    "summary": f"Check {vendor_name} vendor connectivity",
                    "tags": [schema_name],
                    "x-vendor-integration": vendor_name,
        ...
        # Pipeline modules get pipeline-specific OpenAPI paths
        if mod_def.get("module_type") == "pipeline":
            ...
            pipeline_paths = {}
            # List endpoint for job history
            pipeline_paths[resource_path] = {
                "get": {
                    "operationId": f"list_{root_snake}_jobs",
                    "summary": f"List {root} pipeline jobs",
                    "tags": [schema_name],
                    "parameters": [
            ...
            # Trigger endpoint
            trigger_action = "upload" if trigger == "file_upload" else "process"
            pipeline_paths[f"{resource_path}/{trigger_action}"] = {
                "post": {
                    "operationId": f"{trigger_action}_{root_snake}",
                    "summary": f"{'Upload document for' if trigger == 'file_upload' else 'Trigger'} {root} pipeline",
                    "tags": [schema_name],
                    "requestBody": {
            ...
            pipeline_paths[f"{resource_path}/{{id}}/status"] = {
                "get": {
                    "operationId": f"get_{root_snake}_status",
                    "summary": f"Get {root} pipeline status",
                    "tags": [schema_name],
                    "parameters": _id_param,
                    "responses": {"200": {"description": "OK"}},
                },
            }
            pipeline_paths[f"{resource_path}/{{id}}/result"] = {
                "get": {
                    "operationId": f"get_{root_snake}_result",
                    "summary": f"Get {root} pipeline result",
                    "tags": [schema_name],
                    "parameters": _id_param,
                    "responses": {"200": {"description": "OK"}},
                },
            ...
            continue  # skip CRUD paths below
...
def _build_services(modules: dict) -> list:
    ...
    for mod_key, mod_def in modules.items():
        ...
        # Pipeline modules get behavioral endpoints instead of CRUD
        if mod_def.get("module_type") == "pipeline":
            paths = _build_pipeline_paths(
                mod_key, mod_def, svc_name, resource_path, x_source
            )
        elif mod_def.get("build_or_buy") in ("buy", "vendor") or mod_def.get("type") in ("vendor", "buy"):
            # D-PASS4-5 / G6: Buy/vendor-decision modules must NOT generate full CRUD
            # scaffolding — that produces a fake SQLAlchemy model for a third-party SaaS
            # system (e.g., Salesforce). Emit a thin vendor-client stub marker instead so
            # DeterministicCodeGenerator knows to render an SDK wrapper, not a DB model.
            paths = _build_vendor_stub_paths(
                root, root_snake, svc_name, resource_path, x_source,
                vendor=mod_def.get("vendor_name") or root,
            )
        else:
            paths = _build_crud_paths(
                root, root_snake, svc_name, resource_path, x_source
            )
...
def _build_crud_paths(root, root_snake, svc_name, resource_path, x_source):
    """Standard CRUD endpoints for entity modules."""
    from app.modules.solutions_product.services.product_spec_bundle import PathDef
...
def _build_pipeline_paths(mod_key, mod_def, svc_name, resource_path, x_source):
    """Pipeline-specific endpoints derived from BFG steps."""
    from app.modules.solutions_product.services.product_spec_bundle import PathDef
    ...
    # List endpoint for pipeline job history
    paths.append(PathDef(
        path=resource_path, method="GET",
        operation_id=f"list_{mod_key}_jobs",
        summary=f"List {mod_def.get('display_name', mod_key)} pipeline jobs",
        request_schema=None,
        response_schema=svc_name,
        archimate_source_id=x_source,
    ))
    ...
    if trigger == "file_upload":
        paths.append(PathDef(
            path=f"{resource_path}/upload", method="POST",
            operation_id=f"upload_{mod_key}",
            summary=f"Upload document for {mod_def.get('display_name', mod_key)} processing",
            request_schema=None,
            response_schema=svc_name,
            archimate_source_id=x_source,
        ))
        ...
        paths.append(PathDef(
            path=f"{resource_path}/process", method="POST",
            operation_id=f"process_{mod_key}",
            summary=f"Trigger {mod_def.get('display_name', mod_key)} pipeline",
            request_schema=svc_name,
            response_schema=svc_name,
            archimate_source_id=x_source,
        ))
    ...
    # Per-step endpoints for steps that accept input
    for step in steps:
        step_name = step.get("name", "unknown")
        if step.get("service") != "internal":
            continue  # external service steps don't need their own endpoint
        paths.append(PathDef(
            path=f"{resource_path}/{{id}}/{step_name.replace('_', '-')}", method="POST",
            operation_id=f"{mod_key}_{step_name}",
            summary=f"Execute {step_name} step",
            request_schema=svc_name,
            response_schema=svc_name,
            archimate_source_id=x_source,
        ))
...
def _build_vendor_stub_paths(root, root_snake, svc_name, resource_path, x_source, vendor: str = ""):
    """Vendor/buy-decision modules: generate thin SDK-wrapper endpoints only.

    D-PASS4-5 / G6: Instead of full CRUD routes (which would produce a SQLAlchemy model
    for a third-party SaaS system), emit read-only query endpoints and a health-check path.
    The DeterministicCodeGenerator detects x-vendor-integration in the PathDef and renders
    a vendor SDK client wrapper instead of a DB-backed model.
    """
    from app.modules.solutions_product.services.product_spec_bundle import PathDef
    ...
    return [
        PathDef(
            path=resource_path, method="GET",
            operation_id=f"query_{root_snake}_vendor",
            summary=f"[Vendor: {vendor or root}] Query records via SDK",
            request_schema=None,
            response_schema=svc_name,
            archimate_source_id=x_source,
            vendor=True,
        ...
        PathDef(
            path=f"{resource_path}/{{id}}", method="GET",
            operation_id=f"get_{root_snake}_vendor",
            summary=f"[Vendor: {vendor or root}] Get record by ID via SDK",
            request_schema=None,
            response_schema=svc_name,
            archimate_source_id=x_source,
            vendor=True,
        ...
        PathDef(
            path=f"{resource_path}/health", method="GET",
            operation_id=f"{root_snake}_vendor_health",
            summary=f"[Vendor: {vendor or root}] Check vendor connectivity",
            request_schema=None,
...
def _build_confirmed_fields(modules: dict) -> dict:
    ...
    D-PASS5-2: vendor/buy-decision modules are excluded so DeterministicCodeGenerator
    never generates a SQLAlchemy model for a third-party SaaS system. Those modules are
    handled by the post-generation SDK rewrite pass in codegen_routes.py.
    """
    confirmed = {}
    for mod_key, mod_def in modules.items():
        # D-PASS5-2: skip vendor/buy modules — no DB-backed model should be generated
