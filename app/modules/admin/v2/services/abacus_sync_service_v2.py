...
from app.modules.import_batch.services.abacus_import_service import AbacusImportService
...
class AbacusSyncService:
    ...
    async def async_run_full_sync(self) -> Dict[str, any]:
        ...
        try:
            ...
            # Import typed relationships (technology/hosting/ownership connections)
            # These were previously discarded; now routed to ArchiMateRelationship records
            try:
                all_connections = self.connector.extract_all_typed_connections(applications)
                typed_rels = all_connections.get("typed_relationships", [])
                if typed_rels:
                    await self.import_service.import_typed_relationships(typed_rels)
                    logger.info("Imported %d typed relationships from Abacus OutConnections", len(typed_rels))
            except Exception as typed_exc:
                logger.warning("Typed relationship import failed (non-blocking): %s", typed_exc)
            ...
            if self.external_system:
                ...
                # Cache filter dimensions from fetched apps for admin settings page
                try:
                    dims = {}
                    for app_data in applications:
                        for conn in app_data.get("_out_connections", app_data.get("OutConnections", [])):
                            ct = conn.get("ConnectionTypeName", "")
                            sn = conn.get("SinkComponentName", "")
                            if ct and sn:
                                if ct not in dims:
                                    dims[ct] = set()
                                dims[ct].add(sn)
                    if dims:
                        ...
                        if self.external_system.config_json:
                            try:
                                config = _json.loads(self.external_system.config_json)
                            except Exception:
                                config = {}
                        config["_cached_dimensions"] = {k: sorted(v) for k, v in sorted(dims.items())}
                        self.external_system.config_json = _json.dumps(config)
                        logger.info("Cached %d filter dimensions from sync data", len(dims))
                except Exception as exc:
                    logger.debug("Could not cache filter dimensions: %s", exc)
