...
class ARBIntegrationService:
    ...
    def _build_business_case(
        ...
        summary = (
            ...
            f"This capability currently supports {app_count} application(s) "
    ...
    def get_arb_portfolio_summary(self) -> Dict[str, Any]:
        ...
        with_arb_status = len([c for c in capabilities if c.arb_status])
        pending = len([c for c in capabilities if c.arb_status == 'pending_review'])
        approved = len([c for c in capabilities if c.arb_status == 'approved'])
        approved_with_conditions = len([c for c in capabilities if c.arb_status == 'approved_with_conditions'])
        rejected = len([c for c in capabilities if c.arb_status == 'rejected'])
