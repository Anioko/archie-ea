"""
SOPHISTICATED IMPORT ROUTES for unified_applications_bp
...
from ..models.import_audit import ImportAuditService, ImportSessionLog
from ..services.unified_import.duplicate_detector import DuplicateDetector
...
def add_sophisticated_import_routes(blueprint):
    """Add all sophisticated import routes to the provided blueprint"""

    @blueprint.route("/import-fields", methods=["GET"])
    @login_required
    ...
    @blueprint.route("/analyze-import", methods=["POST"])
    @login_required
    @audit_log("import_duplicates_analyze")
    ...
    @blueprint.route("/upload-excel", methods=["POST"])
    @login_required
    @audit_log("import_excel_upload")
    ...
    @blueprint.route("/preview-ai-analysis", methods=["POST"])
    @login_required
    @audit_log("import_ai_preview")
    ...
    @blueprint.route("/import-manual", methods=["POST"])
    @login_required
    @audit_log("import_manual")
    ...
    @blueprint.route("/import-history", methods=["GET"])
    @login_required
    def import_history():
    ...
    def rollback_import_by_session(session_id):
        ...
        try:
            ...
            current_app.logger.error(f"Rollback failed for session {session_id}: {e}")
            return jsonify({"error": f"Rollback failed: {str(e)}"}), 500

    @blueprint.route("/download-template", methods=["GET"])
    @login_required
