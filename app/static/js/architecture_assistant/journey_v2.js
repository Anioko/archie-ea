(function (global) {
    ...
    global.journeyV2Flow = function (solutionId) {
        ...
        return Object.assign({}, vendorMixins, {
            ...
            /**
             * Gather current-step data for the AI Copilot "Enhance All" review.
             * Returns a plain object matching the shape expected by WizardCopilotService.
             */
            _gatherStepData: function (step) {
                switch (step) {
                    case 1:
                        return {
                            problem_statement: this.enrichedBrief || this.problemStatement || '',
                            timeline_months: (this.structuredIntake || {}).timeline_months || null,
                            budget_min: (this.structuredIntake || {}).budget_min || null,
                            budget_max: (this.structuredIntake || {}).budget_max || null,
                            business_domain: (this.structuredIntake || {}).business_domain || '',
                        };
                    case 2:
                        return {
                    ...
                    case 3: {
                        let elements = [];
                        let byLayer = (this.architectureResult || {}).elements_by_layer || {};
                        Object.values(byLayer).forEach(function (arr) {
                    ...
                    case 4:
                        return { decision_points: this.decisionPoints || [] };
                    case 5:
                        return { roadmap: this.roadmapData || {} };
                    case 6:
                        return {
                            section_narratives: (this.arbPackage || {}).section_narratives || {},
                            arb_summary: (this.arbPackage || {}).summary || '',
                        };
                    default:
                        return {};
                }
            ...
            goToStep: function (step) {
                ...
                let hasCode = this.codegenResult || (window.__codegenInit && window.__codegenInit.hasFiles);
                ...
                switch (this.currentStep) {
                    ...
                    case 7:
                        if (this.codegenResult) {
                            ...
                            this.copilotMessage = 'Building your project — this takes 30–60 seconds. Continue reviewing your architecture.';
                        } else if (this.codegenError) {
                            this.copilotMessage = 'Code generation failed. Open "Change language or mode" to retry.';
                        } else {
                            this.copilotMessage = 'Architecture complete. Code generation started — Step 7 will show your project when ready.';
                        }
                        break;
                    case 8:
            ...
            _validateGenerationReadiness: function () {
                // Calls /generation-readiness to surface warnings and block on errors.
                let self = this;
                self.codegenWarnings = [];
                return _fetch(API_BASE + '/' + self.solutionId + '/generation-readiness')
                    .then(function (data) {
                        if (data && data.warnings) {
                            self.codegenWarnings = data.warnings.filter(function (w) {
                                return w.level === 'warning' || w.level === 'info';
                            });
                        }
                        // Hard block only if no elements at all — generation will produce nothing.
                        if (data && data.ready === false) {
                            return Promise.reject({
                                message: (data.warnings && data.warnings.find(function (w) {
                                    return w.level === 'error';
                                })) ? data.warnings.find(function (w) {
                                    return w.level === 'error';
                                }).message : 'No architecture elements found. Complete Step 2 first.',
                            });
                        }
                        return data;
                    })
                    .catch(function (err) {
                        // Network failure → advisory only, don't block generation.
                        if (err && err.message && err.message.includes('No architecture')) {
                ...
                return self._ensureWorkbenchUml()
                    ...
                    .then(function () {
                        let payload = {
                            ...
                            generation_policy: 'production-ready',
                            enforce_chain_complete: self.codegenMode !== 'genome',
                            require_confirmed_specs: self.codegenMode !== 'genome',
            ...
            // Restored 2026-06-11 from 9a2868cf6: a bad merge spliced two
            // implementations together, leaving references to undeclared
            // idx/briefs/errors/processNext — every document upload threw a
            // ReferenceError and left the spinner stuck.
            ...
            getMatchBadgeClass: function (matchType) {
                switch (matchType) {
                    ...
                    case 'novel':
                    case 'new':
                        return 'bg-blue-500/10 text-primary border border-blue-500/30';
                    default:
            ...
            getSourceBadgeClass: function (source) {
                switch (source) {
                    ...
                    case 'derived':
                    case 'ai':
                        return 'bg-purple-500/10 text-primary border border-purple-500/30';
                    case 'inferred':
                        return 'bg-blue-500/10 text-primary border border-blue-500/30';
                    default:
                        return 'bg-zinc-500/10 text-zinc-600 border border-zinc-500/30';
            ...
            getTypeBadgeClass: function (type) {
                ...
                if (t.indexOf('service') !== -1) return 'bg-blue-500/10 text-primary border border-blue-500/30';
                if (t.indexOf('component') !== -1) return 'bg-indigo-500/10 text-primary border border-indigo-500/30';
                if (t.indexOf('node') !== -1 || t.indexOf('device') !== -1) return 'bg-cyan-500/10 text-cyan-600 border border-cyan-500/30';
                if (t.indexOf('role') !== -1 || t.indexOf('actor') !== -1) return 'bg-purple-500/10 text-primary border border-purple-500/30';
            ...
            _domainBadgeClasses: {
                ...
                APP: 'bg-blue-500/10 text-primary border border-blue-500/30',
                DATA: 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/30',
                SEC: 'bg-red-500/10 text-destructive border border-red-500/30',
                DEV: 'bg-cyan-500/10 text-cyan-600 border border-cyan-500/30',
                AI: 'bg-purple-500/10 text-primary border border-purple-500/30',
            ...
            importData: function () {
                ...
                let mappings = this.getCurrentMappings().filter(function (m) { return !!m.target_field; });
            ...
            requestAutoFix: function () {
                ...
                let failedScenarios = (this.testScenarios || []).filter(function (s) { return s.verdict === 'fail'; });
                _fetch('/solutions/' + this.solutionId + '/codegen/test/auto-fix', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
