...
| `untenanted-reads` | a read (`db.select`, `.query`, `session.get`) of ANY model with no `TenantMixin`, with no org predicate in the statement | ratchet @ 2788; a bare `tenant-scoping-ok` (no reason) or an org word inside another name does not clear a read |
