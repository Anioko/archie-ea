...
| W1-4 | **Delete** the never-registered blueprints | SEC-02 | **DONE** `86c1765` — 9 modules + 3 templates deleted (123 unreachable routes), incl. both unauthenticated ones. 4 correctly spared. 3 dangling sidebar referrers fixed (plan recorded 1). |
| W1-5 | Land a `verify.py` gate: **blueprints defined == blueprints registered**, ratcheted at 0, so the class cannot return | audit ethic | TODO — next. Must match the RESOLVED set (`_safe_register`, `_register_optional_standalone`, aliased re-exports), or it reports 3 false positives on run one. |
| W1-7 | Delete `app/main/business_capability_management_routes.py` **together with** `tests/test_business_capability_tenancy.py` (imports `_org_scope` at 4 sites) and the 9 `B608` entries in `.bandit-baseline.json` keyed to it. Declined in W1-4 as a half-deletion. | FC-01 | TODO |
| W1-8 | **`broken-surfaces` is red at 8 and is NOT in `--tag static`** — so a green static run does not cover it. Pre-existing (none of the 7 files has a commit since `5bc3ce7`), *not* caused by W1-4. 1 dead fetch (`duplicate_detection/dashboard.js:71` → 404s), 5 swallowed catches, 2 silent-empty renders. **`--update-baseline` tries to raise this 0 → 8 — never accept that.** Fix the 8, and add the gate to the static tag so it cannot hide again. | CQ-02 | TODO |
