...
| Browser compatibility | Agreed matrix passes: Edge/Chrome, Firefox ESR, Safari if supported | Exact-SHA WebKit passed 56/56. The intermittent Linux/Firefox object was attributed locally to Alpine's benign cancelled-transition sentinel; exact-shape filtering retains arbitrary rejection failures, and the corrected complete local Firefox command passed 56/56. Edge and real Safari remain external. | Proven locally and on WebKit CI; final Linux Firefox rerun required |
