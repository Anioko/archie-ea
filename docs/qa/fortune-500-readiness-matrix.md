...
| Accessibility | WCAG 2.2 AA automated and manual evidence; no critical/serious issue on critical journeys | Linux Chromium exact-SHA audit reports four serious `/ai-chat` contrast failures; improved evidence capture will identify their selectors in the next run. Local Chromium audit passes, confirming platform-sensitive rendering. | Failed—F500-014; manual NVDA/VoiceOver also external |
