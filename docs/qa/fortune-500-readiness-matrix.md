...
| Automated suite integrity | Suite collects with strict markers; no import/collection errors or applicable skips | Skip-producing maintenance/version branches were removed or made explicitly applicable; corrected Chromium suite passed 177/177 with zero non-passes; four CSP browser checks now have Chromium in the main tests job and pass 4/4 locally | Proven locally; final PostgreSQL 16 CI required |
...
| Authorization | Route/API action matrix covers anonymous and every enterprise role | Corrected Chromium suite, typed-ARB authorization regression and runtime-role module pass; security architects can inspect Governance Gates but mutation remains admin-only; the combined post-CI regression cohort passed 26/26 | Proven locally; final CI required |
...
| Browser compatibility | Agreed matrix passes: Edge/Chrome, Firefox ESR, Safari if supported | Exact-SHA WebKit passes. Linux Firefox run 33830528607 failed 3/56 motivation viewports after a local 56/56 pass; tracing found and removed an identical second default-viewpoint data load while preserving real filtered refreshes in a runtime test. Edge and real Safari remain external. | Firefox production fix proven locally; final Linux rerun required |
| Accessibility | WCAG 2.2 AA automated and manual evidence; no critical/serious issue on critical journeys | Linux Chromium exact-SHA run 33808606132 passed the corrected main browser job and both accessibility ratchets after the four Expertise Areas chips moved from a 4.26:1 base-on-tint pair to the approximately 6.14:1 DESIGN.md emphasis pairing. | Automated critical-page audit proven on candidate; manual NVDA/VoiceOver external |
