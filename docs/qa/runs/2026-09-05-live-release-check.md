...
CI 33965073124 final backend result: **4281 passed, 1 failed**, duration 34m18s. Its sole failure is the actual PostgreSQL CSV round-trip case (`test_export_and_import_real_tenant_elements[csv]`) raising the same missing `created_at` AttributeError as the browser download. The subsequent route-verification gate was not reached. Whole run FAILED. The timestamp repair is in the newer 64825fe5 run; its complete result is still pending, so prior passes are not release evidence for the new commit.
