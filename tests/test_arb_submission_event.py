...
def _insert_receipt(cursor, organization_id, user_id, generation=1):
    ...
    cursor.execute(
        ...
        INSERT INTO command_idempotency_records (
            ...
            claim_token, claimant_request_id, attempt_count
        ) VALUES (%s, %s, 'arb.submit', %s, %s, %s, 'in_progress', %s,
                  %s, %s, 1) RETURNING id
        """,
        (organization_id, user_id, token, "a" * 64, f"arb:{token}", generation,
         token, token),
    )
...
def test_direct_sql_accepts_every_exact_typed_submission_shape(app, _schema, subject_type):
    ...
    try:
        ...
        receipt_id = _insert_receipt(cursor, organization_id, user_id)
        _insert_event(
...
def test_submission_event_rejects_foreign_actor_and_stale_generation(app, _schema):
    ...
    try:
        ...
        receipt_id = _insert_receipt(cursor, organization_id, user_id, generation=2)
